...
export async function engineSendText(
  ...
  const qrResult = await sendTextViaQrIfConnected(args);
...
export async function engineSendMedia(
  ...
  const qrResult = await sendMediaViaQrIfConnected(args);
  if (qrResult) return qrResult;
...
async function sendInteractiveViaMeta(
  ...
  const qrResult = await sendInteractiveViaQrIfConnected(input);
  if (qrResult) return qrResult;
