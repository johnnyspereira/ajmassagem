...
export async function POST(request: Request) {
  try {
    ...
    const qrInternalMode = mode === 'qr_internal';

    if (qrInternalMode) {
      return sendQrInternalBroadcast({
        supabase,
        accountId,
        userId: user.id,
        recipients,
        internalKind: internal_kind === 'interactive' ? 'interactive' : 'text',
      });
    }
...
type BroadcastSupabase = Awaited<ReturnType<typeof createClient>>;

async function sendQrInternalBroadcast({
  supabase,
  accountId,
  userId,
  recipients,
  ...
  supabase: BroadcastSupabase;
  accountId: string;
  userId: string;
  recipients: NewRecipient[];
  internalKind: 'text' | 'interactive';
}) {
  const qrStatus = await waitForQrConnection(accountId, userId);
  ...
  for (const recipient of recipients) {
    ...
    if (!contactId) {
      results.push({
        phone: recipient.phone,
        status: 'failed',
        error: 'contact_id is required for internal QR broadcasts',
      });
      failedCount++;
      continue;
    }
    ...
    const phone = String(contact.phone ?? recipient.phone ?? '');
    ...
    if (!isValidE164(sanitizedPhone)) {
      results.push({
        ...
        status: 'failed',
        error: 'Invalid phone number format. Use a full country code number.',
      });
      failedCount++;
      continue;
    }

    const text = renderQrInternalText(recipient, contact, internalKind);
    if (!text) {
      results.push({
        phone,
        contact_id: contact.id,
        status: 'failed',
        error: 'Internal template has no text to send.',
      });
      failedCount++;
      continue;
    }
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : 'Unknown error';
      console.error(
        `Failed to send internal broadcast to ${phone}:`,
        errorMessage
      );
      results.push({
        phone,
...
function renderQrInternalText(
  ...
  const rawText =
    internalKind === 'interactive' && recipient.interactive_payload
      ? interactivePayloadToText(recipient.interactive_payload)
      : recipient.content_text || '';
  ...
  return rawText
    .replace(/\{\{\s*(name|phone|email|company)\s*\}\}/gi, (_, key) => {
      return fields[String(key).toLowerCase()] || '';
    })
    .trim();
}
...
async function findOrCreateConversation(
  ...
  if (findError) {
    console.error('Error resolving broadcast conversation:', findError.message);
    return null;
  }
  ...
  if (error || !created) {
    console.error(
      'Error creating conversation for internal broadcast:',
      error?.message
    );
    return null;
  }
