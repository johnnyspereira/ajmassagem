...
export async function GET(request: Request) {
  ...
  if (!state || state !== expected || !code || !auth || auth.profile.account_role !== 'owner') return NextResponse.redirect(new URL('/website?google=failed', origin));
  ...
  try {
    ...
    if (!tokenResponse.ok || !token.refresh_token) throw new Error('Google did not return a refresh token.');
  ...
  } catch { return NextResponse.redirect(new URL('/website?google=failed', origin)); }
}
