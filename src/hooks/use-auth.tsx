  ...
  const fetchProfile = useCallback(async (userId: string) => {
    ...
    try {
      ...
      if (data) {
        ...
        if (data.account_id) {
          const localPreferences = readLocalAccountPreferences(data.account_id);
          ...
          if (accountErr) {
            ...
            accountRow = {
              ...
              crm_locale:
                account.crm_locale === 'en' || account.crm_locale === 'pt'
                  ? account.crm_locale
                  : (localPreferences.crm_locale ?? 'pt'),
              timezone:
                typeof account.timezone === 'string' && account.timezone
                  ? account.timezone
                  : (localPreferences.timezone ?? 'Europe/Lisbon'),
              public_url:
                typeof account.public_url === 'string'
                  ? account.public_url
                  : (localPreferences.public_url ?? null),
              navigation_layout:
                account.navigation_layout === 'topbar' ||
                account.navigation_layout === 'sidebar'
                  ? account.navigation_layout
                  : (localPreferences.navigation_layout ?? 'sidebar'),
              logo_url:
                typeof account.logo_url === 'string'
                  ? account.logo_url
                  : (localPreferences.logo_url ?? null),
            };
          }
        }
