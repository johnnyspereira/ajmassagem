...
export function PublicServiceExplorer({ services, bookingUrl }: { services: PublicService[]; bookingUrl: string }) {
  ...
  return <>
    ...
    <Dialog open={Boolean(selected)} onOpenChange={(open) => !open && setSelected(null)}>
      {selected && <DialogContent className="max-h-[90vh] overflow-y-auto p-0 sm:max-w-3xl"><div className="grid sm:grid-cols-[.9fr_1.1fr]">{selected.public_image_url ? <img src={selected.public_image_url} alt={selected.name} className="h-56 w-full object-cover sm:h-full" /> : <div className="hidden bg-gradient-to-br from-violet-100 via-white to-emerald-100 sm:flex sm:min-h-96 sm:items-center sm:justify-center"><Sparkles className="size-14 text-[var(--brand)]" /></div>}<div className="p-6 sm:p-8"><DialogHeader><p className="text-sm font-semibold text-[var(--brand)]">{selected.duration_minutes} minutos · {Number(selected.price) > 0 ? new Intl.NumberFormat('pt-PT', { style: 'currency', currency: selected.currency || 'EUR' }).format(Number(selected.price)) : 'Valor sob consulta'}</p><DialogTitle className="text-3xl">{selected.name}</DialogTitle><DialogDescription className="text-base leading-7 text-slate-600">{selected.public_presentation || selected.description || 'Uma experiência pensada para o seu bem-estar.'}</DialogDescription></DialogHeader><DetailList icon={Check} title="Benefícios" items={selected.public_benefits} empty="Alívio de tensão, bem-estar e cuidado personalizado." tone="emerald" /><DetailList icon={TriangleAlert} title="Cuidados e contraindicações" items={selected.public_considerations} empty="Informe-nos sobre lesões, gravidez, condições de saúde ou desconforto antes do atendimento." tone="amber" /><div className="mt-7 rounded-xl bg-slate-50 p-4 text-sm text-slate-600"><Info className="mr-2 inline size-4 text-[var(--brand)]" />A avaliação é individual. Adaptamos a sessão às suas necessidades e ao seu conforto.</div><Button className="mt-5 w-full" render={<a href={bookingUrl} />} >Marcar esta experiência <ArrowRight /></Button></div></div></DialogContent>}
    </Dialog>
  </>;
}

function DetailList({ icon: Icon, title, items, empty, tone }: { icon: typeof Check; title: string; items?: string[] | null; empty: string; tone: 'emerald' | 'amber' }) { const entries = items?.filter(Boolean).length ? items.filter(Boolean) : [empty]; return <section className="mt-6"><h4 className="flex items-center gap-2 font-semibold"><span className={tone === 'emerald' ? 'rounded-lg bg-emerald-100 p-1.5 text-emerald-700' : 'rounded-lg bg-amber-100 p-1.5 text-amber-700'}><Icon className="size-4" /></span>{title}</h4><ul className="mt-3 space-y-2 text-sm leading-6 text-slate-600">{entries.map((item) => <li key={item} className="flex gap-2"><span className="mt-2 size-1.5 shrink-0 rounded-full bg-slate-400" />{item}</li>)}</ul></section>; }
