...
export function MessageComposer({
  ...
  // QR/Web sends are not bound to Meta's 24h customer-care window, so
  // the composer only gates by role. The warning banner can still
  // explain the Meta limitation, but it should not block QR sends.
  const textInputsDisabled = readOnly;
  const metaInputsDisabled = readOnly;
  ...
  const sendInteractive = useCallback(() => {
    ...
    onSend(interactivePayloadToText(interactivePayload), replyTo?.id);
    setInteractiveOpen(false);
    onClearReply?.();
  }, [interactivePayload, onSend, replyTo?.id, onClearReply]);
  ...
  // builder pre-filled so the agent can tweak before sending as QR text.
  ...
  const startRecording = useCallback(async () => {
    if (metaInputsDisabled || busy || recording) return;
    if (
  ...
  }, [metaInputsDisabled, busy, recording, finalizeRecording]);
    ...
    <div className="border-border bg-card border-t p-3">
      ...
      {draft ? (
        ...
        <div className="flex items-end gap-2">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger
              disabled={metaInputsDisabled || busy}
              title={
                readOnly
                  ? t('readOnlyTitle')
                  : metaInputsDisabled
