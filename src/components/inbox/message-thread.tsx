...
type ThreadTimelineTone = 'note' | 'deal' | 'tag' | 'automation';
...
interface ThreadTimelineEvent {
  ...
  detail: string;
  createdAt: string;
  tone: ThreadTimelineTone;
}

type ThreadDeal = Deal & { stage?: PipelineStage | null };
type ThreadContactTag = {
  id: string;
  created_at: string;
  tags?: Tag | null;
};
type ThreadAutomationLog = {
  id: string;
...
function threadTimelineIcon(tone: ThreadTimelineTone) {
  switch (tone) {
    case 'deal':
      return Briefcase;
    case 'tag':
...
function threadTimelineToneClass(tone: ThreadTimelineTone): string {
  switch (tone) {
    case 'deal':
...
export function MessageThread({
  ...
  useEffect(() => {
    ...
    (async () => {
      const [notesRes, dealsRes, tagsRes, automationRes] = await Promise.all([
        supabase
          .from('contact_notes')
          .select('*')
      ...
      for (const note of (notesRes.data as ContactNote[] | null) ?? []) {
        events.push({
          id: `note-${note.id}`,
          title: 'Note created',
      ...
      for (const deal of (dealsRes.data as ThreadDeal[] | null) ?? []) {
        events.push({
          id: `deal-${deal.id}`,
          title: 'Deal updated',
          detail: `${deal.title}${deal.stage?.name ? ` - ${deal.stage.name}` : ''}`,
          createdAt: deal.updated_at ?? deal.created_at,
          tone: 'deal',
        });
      }
      for (const tag of (tagsRes.data as ThreadContactTag[] | null) ?? []) {
        if (!tag.tags) continue;
        events.push({
          id: `tag-${tag.id}`,
          title: 'Tag added',
          detail: tag.tags.name,
          createdAt: tag.created_at,
          tone: 'tag',
        });
      }
      for (const log of (automationRes.data as ThreadAutomationLog[] | null) ??
        []) {
        events.push({
          id: `automation-${log.id}`,
          title: 'Automation fired',
          detail: `${log.automation?.name ?? log.automations?.name ?? log.trigger_event} - ${log.status}`,
  ...
  }, [contactId, conversationId, resyncToken]);
  ...
  const markOptimisticMessageSent = useCallback(
    (tempId: string, payload: Record<string, unknown>) => {
      ...
      if (typeof payload.whatsapp_message_id === 'string') {
        updates.message_id = payload.whatsapp_message_id;
      }
      onUpdateMessage(tempId, updates);
    },
    [onUpdateMessage]
  );
  ...
  const handleSendMedia = useCallback(
    async (payload: SendMediaPayload) => {
      ...
      try {
        ...
        markOptimisticMessageSent(tempId, data);
      } catch (err) {
        ...
        <div className="flex items-center gap-2">
          ...
          {onToggleContactPanel && (
            <button
              ...
              className={cn(
                'hover:bg-muted hover:text-foreground hidden h-7 w-7 items-center justify-center rounded-md transition-colors xl:inline-flex',
          ...
          {onOpenContactProfile && (
            <button
              ...
              className="text-muted-foreground hover:bg-muted hover:text-foreground inline-flex h-7 w-7 items-center justify-center rounded-md transition-colors xl:hidden"
            >
          ...
          <div className="border-border bg-card/95 mb-4 rounded-md border p-3 shadow-sm">
            ...
            <div className="grid gap-2 sm:grid-cols-2">
              {threadTimelineEvents.map((event) => {
                const Icon = threadTimelineIcon(event.tone);
                return (
                  <div
                    key={event.id}
                  ...
                  >
                    <Icon
                      className={cn(
                        'mt-0.5 h-3.5 w-3.5',
                        threadTimelineToneClass(event.tone)
                      )}
                    />
                    <div className="min-w-0">
